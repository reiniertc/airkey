...
- Diagnostic sensors for monitoring the integration itself: **API requests today** (with `limit`/`remaining` attributes), **main data last refreshed**, and **lock details last refreshed** — see [API request budget & monitoring](#api-request-budget--monitoring) below.
...
- A device per lock, with a sensor exposing door name, type, technology, firmware versions, a "removal requested" binary sensor, and who last unlocked it and when (`last_used_at`/`last_used_by_medium_name`/`last_used_by_person_id`/`last_used_by_person_name`, looked up from the lock's protocol history for the last 30 days).
...
Afterwards, use the integration's **Configure** button to change the polling interval (default 720 minutes / 12 hours, minimum 5), the event lookback window used on the very first refresh (default 24 hours), the lock usage refresh interval (default once a day - see below), and the daily API request limit used for the monitoring sensor below (default 250 - see below).

### API request budget & monitoring

EVVA support confirmed the Airkey Cloud API enforces a daily request quota of **250 requests per account**, resetting at midnight UTC. This isn't documented publicly and may differ per plan, so it's configurable via **Configure** rather than hardcoded.

The main polling cycle costs a fixed number of calls regardless of how many locks you have, but the per-lock "last used" and area-assignment lookups each cost one extra API call per lock, per refresh. To keep the daily total predictable regardless of scan interval, those two are refreshed on their own interval - **once a day by default** - instead of every polling cycle. This once-a-day timestamp is persisted to disk, so a Home Assistant restart doesn't reset it and force an early re-fetch. Use the `airkey.refresh_lock_details` service (or lower the interval in **Configure**) if you want that data sooner than the next scheduled refresh.
...
**Monitoring the quota from within Home Assistant:**

- **API requests today** — how many requests this Home Assistant instance has made to the Airkey API since midnight UTC. `limit` and `remaining` attributes reflect the configured daily limit (default 250, adjustable in **Configure**). This count is local to this integration instance: it can't see requests made elsewhere (another app, another Home Assistant instance using the same API key), and it survives Home Assistant restarts (persisted to disk and reseeded on startup) but resets to 0 whenever the UTC date rolls over.
- **Main data last refreshed** — the timestamp of the most recent successful main polling cycle (persons/locks/areas/media/authorizations/etc.).
- **Lock details last refreshed** — the timestamp of the most recent per-lock "last used"/area-assignment refresh (the once-a-day cycle described above, not the main polling cycle).
